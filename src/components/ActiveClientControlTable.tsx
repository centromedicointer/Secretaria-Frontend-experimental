...
export const ActiveClientControlTable = () => {
  const { toast } = useToast();
  const [searchTerm, setSearchTerm] = useState('');
  
  const { data: clientControlData, isLoading, refetch } = useQuery({
    queryKey: ['active-client-control-details'],
    queryFn: async () => {
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-center text-gray-500 py-8">
            No hay clientes con bot activado
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <User className="h-5 w-5" />
          Clientes con Bot Activado ({filteredData.length})
        </CardTitle>
      </CardHeader>
      <CardContent>
        <PhoneSearchInput
